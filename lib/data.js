...
export const useAssignmentStore = create(
  persist(
    (set, get) => ({
      ...
      getAssignmentsByDepartment: (departmentId) => {
        const { assignments } = get()

        // Look through all exam assignments
        let departmentAssignments = []

        // Iterate through all exams
        Object.values(assignments).forEach((examAssignments) => {
          // Get assignments for this department
          const deptAssignments = examAssignments[departmentId] || []
          departmentAssignments = [...departmentAssignments, ...deptAssignments]
        })
